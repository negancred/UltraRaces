...
public class Goddess extends Race {
    ...
    public Goddess(UltraRaces ultraRaces) {
        super(ultraRaces);
    }
    ...
    public boolean ShouldActivateItemSkill(Player player) {
        return player.getInventory().getItemInMainHand().getType() == Material.NAUTILUS_SHELL;
    }

    @Override
    public void onDamageTaken(Player damagedPlayer, Entity damager, EntityDamageByEntityEvent event) {
        if (damager instanceof LivingEntity living) {
            living.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 60, 2));
            damagedPlayer.getWorld().spawnParticle(Particle.END_ROD, damagedPlayer.getLocation(), 30);
        }
        for (Player goddess : Bukkit.getOnlinePlayers()) {
            if (goddess == damagedPlayer) continue;
            if (!isPlayerThisRace(goddess)) continue;
            ...
            damagedPlayer.addPotionEffect(new PotionEffect(
                    PotionEffectType.REGENERATION, REGEN_DURATION, newLevel - 1, true, true));
    ...
    public void ActivateActiveSkill(Player player) {
        if (!isPlayerThisRace(player)) return;

        UUID uuid = player.getUniqueId();
        long now = System.currentTimeMillis();

        if (isOnCooldown(player, "Banquet", false)) return;
        ...
        SpawnParticlesEffects(player);
        ConduitEffects(conduit);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < points; i++) {
                    SpawnCircularRainParticle(world, center, angleOffset, RADIUS, i, points);
                }
        ...
        for (Player nearby : player.getWorld().getPlayers()) {
            ...
            SettingPlayersToMaxHealth(nearby);
        }

        COOLDOWN.put(uuid, now);
    }

    private void ConduitEffects(ArmorStand conduit) {
    ...
    private void SpawnParticlesEffects(Player player) {
    ...
    private void SpawnCircularRainParticle(World world, Location center, double angleOffset, double radius, int i, int points) {
    ...
    private void SettingPlayersToMaxHealth(Player nearby) {
        nearby.setHealth(Objects.requireNonNull(nearby.getAttribute(Attribute.MAX_HEALTH)).getValue());
        nearby.sendMessage(Component.text("§d§oYou received the blessing of the Goddess..."));
    ...
    private void PlayersInRadiusEffects(Player nearby) {
